...
Do **not** put this text file `pg42671.txt` in 
...
number of lines to read from the beginning of the file and feed into the pipe. 
Then `"$3"` is another argument that specifies the number of lines to print from
the end of text that was fed into the pipe. `"$1"` is an argument that specifies
the name of the file we want to use in the function.

Why do we need the first line of the shell script?  
**Solution:**. 
The first line on the shell script `#!/bin/bash or #!/bin/sh` is known as 
‘she-bang‘(shabang).   
`/bin/sh` is the system shell and `#!/bin/sh` instructs the program loader to 
use bin/sh program instead of any other passing the path of the script as the 
first argument.  
`bin/bash` is the most common shell used and `#!/bin/bash` instructs the program
loader to use /bin/bash program.
