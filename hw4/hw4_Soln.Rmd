...
icu <- readRDS("icu_cohort_HW4.rds") %>% 
  select(-dischtime, -deathtime, -discharge_location, -edregtime, edouttime, -hospital_expire_flag, -outtime, -anchor_year_group, -dod) 

icu <- icu %>% 
  distinct(subject_id, .keep_all = TRUE)

icu_vars <- colnames(icu)
icu_vars[27:36] <- c(
    "hematocrit_lab",
    "sodium_lab", 
    "WBC_lab", 
...
icu_noOutliers <- remove_outliers_IQR(1)

#may need this later
count_NAs <- function(x, df){
  num_NAs <- which(is.na(df[, x]) == T) %>% 
  length()
}
